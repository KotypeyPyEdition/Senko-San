...
class Osu(commands.Cog):
    ...
    async def profile(self, ctx, *, usr=None):
        if not usr:
            return await ctx.send('Usage: sen!osu_profile nickane,')
    ...
    async def best(self, ctx, User=None):
        if not User:
            return await ctx.send('usage: sen!best nicname')
        data = self.utils.best_user_dict(User)
    ...
    async def last(self, ctx, User=None):
        if not User:
            return await ctx.send('usage: sen!last nicname')
        data = self.utils.user_last(User)
        data1 = self.utils.get_map(data['beatmap_id'])
